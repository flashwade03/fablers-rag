...
ほとんどのClaude Code向けRAGソリューションはMCPサーバーです — ドキュメントを埋め込み、top-kチャンクを取得し、コンテキストウィンドウに貼り付けます。簡単な検索には機能しますが、本格的な質問には不十分です。
...
唯一の外部API呼び出しは、検索時のクエリ埋め込み用OpenAI `text-embedding-3-small`だけです。それ以外のすべて — クエリ分析、リランキング、検証、回答合成 — はClaude Code自体のエージェントシステムで実行されます。追加LLMコストなし。

**要約**: MCPは検索結果を返します。このプラグインは検証済みの、引用付き回答を返します — せっかく課金しているClaude Codeのトークン、気持ちよく使いましょう。

### こんな用途にぴったり

このプラグインは個別ドキュメントへの深いQ&A用に作られています — 数百万件のインデックス用ではありません。
...
numpy配列とインメモリBM25を使用 — ベクトルDB不要、サーバー不要。ファイル数個規模ならこれだけで十分です。エンタープライズ規模のナレッジベースには専用ベクトルDBソリューションをお使いください。
