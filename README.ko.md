...
### LLM 리랭킹, 알고리즘 리랭킹이 아닙니다

Evaluator 에이전트는 패시지를 직접 읽고 관련성을 채점하는 LLM 기반 리랭킹을 수행합니다. v1.x 벤치마크에서 알고리즘 리랭킹(cross-encoder 방식)을 hybrid search 위에 적용했을 때, 오히려 성능이 하락했습니다:

| 메트릭 | Hybrid | Hybrid + 알고리즘 Rerank | Δ |
|--------|--------|------------------------|---|
| Hit@1 | 74.4% | 60.0% | -14.4% |
| MRR | 0.811 | 0.712 | -0.099 |

알고리즘 리랭커가 hybrid search에서 이미 잘 정렬된 결과를 다시 섞으면서, 정답을 뒤로 밀어버렸습니다. LLM 기반 리랭킹은 질문-패시지 관계를 의미적으로 이해하므로 이 문제가 발생하지 않습니다.
